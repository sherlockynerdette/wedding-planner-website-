    ...
    public partial class Payment : System.Web.UI.Page
    {
            ...
            if (Request.Cookies["CartSID"] != null)
            {
                ...
                if (CookieDataArray.Length > 0)
                {
                    ...
                    for (int i = 0; i < CookieDataArray.Length; i++)
                    {
                        ...
                        {
                            using (SqlCommand cmd = new SqlCommand(SID + "", con))
                            {
                        ...
                        CartTotal += Convert.ToInt64(["SPrice"]);
        ...
        protected void btnPaytm_Click(object sender, EventArgs e)
        {
            if (Session["USERNAME"] != null)
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
                {
                    SqlCommand cmd = new SqlCommand("insert into tblPurchase values('" + USERID + "','" + hdCartAmount.Value + "','"
        ...
        private void BindOrderProducts()
        {
            string UserIDD = Session["USERID"].ToString();
            DataTable dt = new DataTable();
            using (SqlConnection con0 = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
            {
                ...
                using (SqlDataAdapter sda0 = new SqlDataAdapter(cmd0))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        foreach (DataColumn SID in dt.Columns)
                        {
                            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
                            {
                                using (SqlCommand cmd = new SqlCommand("SELECT * FROM tblCart C WHERE C.SID=" + SID + " AND UID ='" + UserIDD + "'", con))
                                {
                                    cmd.CommandType = CommandType.Text;
                                    using (SqlDataAdapter sda = new SqlDataAdapter(cmd))
                                    {
        ...
        private void InsertOrderServices()
        {
            string USERID = Session["USERID"].ToString();
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
            {
                ...
                {
                    SqlCommand myCmd = new SqlCommand("SP_InsertOrderServices", con)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
                    myCmd.Parameters.AddWithValue("@OrderID", OrderNumber.ToString());
                    myCmd.Parameters.AddWithValue("@UserID", USERID);
                    myCmd.Parameters.AddWithValue("@SID", gvr.Cells[0].Text);
                    ...
                    if (con.State == ConnectionState.Closed) { con.Open(); }
                    Int64 OrderSerID = Convert.ToInt64(myCmd.ExecuteScalar());
                    con.Close();
                    EmptyCart();
