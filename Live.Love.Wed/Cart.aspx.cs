    ...
    public partial class Cart : System.Web.UI.Page
    {
        ...
        private void BindServiceCart()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
            {
                ...
                {
                    sda.Fill(dt);
                    RptrCartServices.DataSource = dt;
                    RptrCartServices.DataBind();
                    if (dt.Rows.Count > 0)
                    {
        ...
        protected void RptrCartServices_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                int CartSID = Convert.ToInt32(e.CommandArgument.ToString().Trim());
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LiveLoveWedDB"].ConnectionString))
                {
                    ...
                    myCmd.Parameters.AddWithValue("@CartID", CartSID);
                    con.Open();
